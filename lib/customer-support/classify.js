...
export async function processEmail({
  ...
  console.log(
    `Gemini Result: ${classificationResult.classification} (Confidence: ${(classificationResult.confidence * 100).toFixed(0)}%)`
  );
  ...
  const queueStatus = queueStatusForClassification(classificationResult.classification);
 ...
 */
async function archiveToSupportFolder(client, uid) {
  const targetMailbox = 'WisdomPackets Support';
  try {
  ...
  try {
    ...
    console.log(`Moved email UID ${uid} to '${targetMailbox}' folder in Gmail.`);
  } catch (moveErr) {
    console.error(`Failed to move email UID ${uid} to '${targetMailbox}':`, moveErr.message);
  }
}
...
export async function runClassifier() {
  ...
  console.log(`Connecting to IMAP mailbox: ${host}...`);
    ...
    for (const mailbox of mailboxesToSearch) {
      ...
      for (const uid of uids) {
        try {
          ...
          if (mailbox === 'INBOX') {
            await archiveToSupportFolder(client, uid);
          }
