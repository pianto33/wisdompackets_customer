...
import { findCancelableSubscription } from './stripe-utils.js';
  ...
  if (stripe && ['Desuscripción', 'Desuscripción + Refund'].includes(classificationResult.classification)) {
    ...
    try {
      ...
      if (customers.data.length > 0) {
        ...
        const activeSub = findCancelableSubscription(subscriptions.data);

        if (activeSub) {
          stripeSubscriptionId = activeSub.id;
          stripeTag = 'Found';
          console.log(`Stripe cancelable subscription found: ${activeSub.id} (Status: ${activeSub.status})`);
